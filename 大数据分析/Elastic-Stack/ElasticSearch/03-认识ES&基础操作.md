...
Elasticsearch集群可以包含多个**索引(indices)**（数据库表）。
...
用户在ES中**存储的数据文档**，每一个文档都有唯一的标识（_id），id可以自行指定或ES自动生成！！
...
每个索引都有多个分片，每个**分片是一个Lucene索引**；将索引划分成多份的能力，这些份就叫做分片！！！
...
拷贝一份分片就完成了**分片的备份**。在一个网络/云的环境里，失败随时都可能发生，在某个分片/节点不知怎么的就处于离线状态，或者由于任何原因消失了。这种情况下，有一个故障转移机制是非常有用并且是强烈推荐的。为此目的，Elasticsearch允许你创建分片的一份或多份拷贝，这些拷贝叫做复制分片，或者直接叫复制。
...
ILM一方面简化了滚动索引的定时调用的情况；另一方面控制了index中数据的生命周期！！！

Index Lifecycle Management(索引生命周期管理，后文简称 ILM)在6.6 第一次被引入beta版本，在6.7里GA版本，它内部实现了cron 的工作，我们只需要去配置它的policy即可。

ILM一共将索引生命周期分为四个阶段 (Phase)：Hot 阶段、Warm 阶段、Cold 阶段、Delete 阶段
...
# index.lifecycle.rollover_alias 指明在 Rollover 的时候使用的alias
...
# 4、ILM Service会在后台轮询执行Policy，默认间隔时间为10分钟，为了更快地看到效果，我们将其修改为1秒。
...
非结构化创建语法：PUT prod_search_info

查看所有的indices：GET _cat/indices

查看创建后的结构：GET prod_search_info，可以发现mappings中是空的！

```json
# create index
PUT /prod_search_info

# 查看所有索引
GET _cat/indices

# delete index
DELETE prod_search_info
```

### 2.3.2 结构化创建

结构化创建语法：PUT prod_search_info { // 对应信息 }
...
修改语法：PUT /school/_doc/mapping

### 2.3.6 查看索引

查看有哪些索引语法：GET /_cat/indices

查看指定索引的结构：GET /prod_search_info/_mapping
...
语法：put /prod_search_base/_doc/1   {"name":"guo","age":22}

```json
# create document
PUT /prod_search_base/_doc/1
{
  "username": "alfred",
  "age": 1
}
...
语法：POST /prod_search_base/_doc   {"name":"guo","age":22}

```json
# create document without id
POST /prod_search_base/_doc
{
  "username": "tom",
  "age": 20
}
...
语法：get /prod_search_base/_doc/1

```json
# 指定id查询
GET /prod_search_base/_doc/1

# 查询整个索引
GET /prod_search_base/_search
...
# 批量操作 bulk api
POST _bulk
{"index":{"_index":"prod_search_base","_id":"1"}}
{"name":"alfred","age":20}
{"delete":{"_index":"prod_search_base","_id":"2"}}
{"update":{"_id":"3","_index":"prod_search_base"}}
{"doc":{"age":"20"}}
{"create":{"_index":"prod_search_base","_id":"4"}}
{"name":"guo","age":24}
```
...
```properties
# 指定文档id更新
POST /prod_search_base/_update/1
{
  "script": {
...
# 全量更新
POST /prod_search_base/_update_by_query
{
  "script": {
...
POST /prod_search_info/_update/1
{
  "script": {
    "lang": "painless",
    "source": """
...
```properties
# 根据id删除
DELETE /prod_search_base/_doc/1

# 指定条件删除
POST /prod_search_base/_delete_by_query
{
  "query": {
    "match": {
      "username": "郭东升"
