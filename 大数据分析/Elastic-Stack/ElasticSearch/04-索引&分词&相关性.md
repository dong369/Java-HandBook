...
通过文档id=>文档内容、单词的关联关系，一本书的目录对应的就是正排索引。
...
通过单词=>文档id的关联关系，通过正排索引进行分词形成倒排索引，一本书的索引页对应的是倒排索引。
...
倒排列表（Posting List）记录了单词对应的文档集合，由倒排索引项（Posting）组成，倒排索引项（ Posting）主要包含如下信息：
...
可以通过explain参数来查看具体的计算方法。但要注意ES的算分是按照shard进行的，即shard的分数计算是相互独立的，所以在使用explain的时候注意分片数，可以通过设置索引的分片数为1来避免这个问题。TF/DF模型是Lucene的经典模型。
